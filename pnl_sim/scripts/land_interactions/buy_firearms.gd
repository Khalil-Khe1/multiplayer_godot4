...
func on_pressed(player : Player, land : Turf):
	self.button.pressed.connect(
		func ():
			...
			var dialog = "\n[center]Buy firearms\n"
			dialog = dialog + "[center]price: " + land.get_inventory()["firearms"][1] + "per unit"
			...
			interact_btn.pressed.connect(
				func ():
					...
					if(!value.is_valid_int()):
						return
					value = int(value)
					if(player.get_resources()["dirty"] < value * land.get_inventory()["firearms"][1]):
						return
					player.append_resource("firearms", value)
					player.deplete_resource("dirty", value * land.get_inventory()["firearms"][1])
