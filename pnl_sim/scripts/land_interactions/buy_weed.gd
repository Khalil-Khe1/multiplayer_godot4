			...
			interact_btn.pressed.connect(
				func ():
					...
					if(!value.is_valid_int()):
						return
					value = int(value)
					if(player.get_resources()["dirty"] < value * land.get_inventory()["weed"][1]):
						return
					player.append_resource("weed", value)
					player.deplete_resource("dirty", value * land.get_inventory()["weed"][1])
