...
func on_pressed(player : Player, land : Turf):
	self.button.pressed.connect(
		func ():
			...
			var dialog = "\n[center]Sell firearms"
			dialog = dialog + "[center]price: " + land.get_inventory()["firearms"][0] + "per unit"
			...
			interact_btn.pressed.connect(
				func ():
					...
					if(!value.is_valid_int()):
						return
					value = int(value)
					if(player.get_resources()["firearms"] < value):
						return
					player.append_resource("dirty", value * land.get_inventory()["firearms"][0])
					player.deplete_resource("firearms", value )
